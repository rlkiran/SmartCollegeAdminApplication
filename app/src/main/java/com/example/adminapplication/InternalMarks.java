...
public class InternalMarks extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        up_bt = findViewById(R.id.bt_upIT);
        up_lt = findViewById(R.id.layout_upIT);
        upload_file = findViewById(R.id.bt_file_select_it);
        img_IT = findViewById(R.id.img_internals);
        sp1 = findViewById(R.id.it_year_sp);
        sp2 = findViewById(R.id.it_branch_sp);
        sp3 = findViewById(R.id.it_section_sp);
        sp4 = findViewById(R.id.it_sp4);
        sp5 = findViewById(R.id.it_sp5);
        sp6 = findViewById(R.id.it_sp6);
        up_cancel = findViewById(R.id.bt_upCancel_it);
        up_ok = findViewById(R.id.bt_upOk_it);
        del_lt = findViewById(R.id.layout_delIT);
        del_bt = findViewById(R.id.bt_delIT);
        del_cancel = findViewById(R.id.bt_del_cancel_it);
        del_ok = findViewById(R.id.bt_del_ok_it);
        ...
        del_cancel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) { del_lt.setVisibility(View.INVISIBLE);
            }
        });
        del_ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(filename!= null) {
                    tt_storageReference = FirebaseStorage.getInstance().getReference("InternalMarks").child(filename);
                    tt_databaseReference = FirebaseDatabase.getInstance().getReference("InternalMarks").child(filename);
                    tt_storageReference.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {

                            DatabaseReference ref = FirebaseDatabase.getInstance().getReference("InternalMarks").child(filename);
                            ...
                            Toast.makeText(InternalMarks.this, "Deleted Successfully", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception exception) {
                            // Uh-oh, an error occurred!
                            Toast.makeText(InternalMarks.this, "Unable To delete file" +exception.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
                }else {
    ...
    private void uploadInternals() {
        if (ImageUri != null)
        {
            final String filename = getDataFromSpinners();
            if(filename!= null) {
                tt_storageReference  = FirebaseStorage.getInstance().getReference("InternalMarks").child(filename);
                tt_databaseReference = FirebaseDatabase.getInstance().getReference("InternalMarks");
                ...
                }).addOnCompleteListener(new OnCompleteListener<Uri>()
                {
                    ...
                    public void onComplete(@NonNull Task<Uri> task)
                    {
                        if (task.isSuccessful()) {
                            Toast.makeText(InternalMarks.this, "File Uploaded Successfully", Toast.LENGTH_SHORT).show();
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(InternalMarks.this, "upload failed: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            } else {
